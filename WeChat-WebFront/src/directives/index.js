...
let va = {
	...
	toast(name, ERR_MSG, type) {
		if(name == "密码" && type == "reg") {
			return utils.toast(name + '请输入6-18位数字与字母组合');
		}
		return utils.toast(ERR_MSG[type]);
	},
...
Vue.directive('va', {
	bind: function(el, binding, vnode) {
		...
		var eazyNew = (type, typeVal) => {
			return new va.VaConfig(type, typeVal, '', name, tag)
		}
		var regNew = (typeVal) => {
			return new va.VaConfig('reg', typeVal, regMsg, name, tag)
		} //正则的新建

		el.className += ' va' + vm._uid; //表示加入原先的样式+ 现在生成的样式
		//el.name = name;//因为div没有默认的name属性，必须设置下2018-01-30
		typeof(el.name) == "undefined" ? el.setAttribute("name", name): (el.name = name);
		...
		if(option.vanow) {
			el.addEventListener('blur', function() {
				...
				if(_result) {
					typeof _result === 'string' ? utils.toast(_result) : va.showErr(conditions[0].tag, _result)
					return;
				}
				vm.vaVal[name] = value
				vm.$vanow(para)
